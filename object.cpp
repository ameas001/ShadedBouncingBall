...
void Player::updatePlayer(double x, double y, std::vector <Line2D> lines)
{
    double rx, ry, p1x, p2x, p1y, p2y;
    double r = c.radius;
    vx += x;
    vy += y;
    ...
    for(int i = 0; i < lines.size(); ++i)
    {
        ...
        if(flag && (p1x + rx) <= c.p.x + vx && (p2x - rx) >= c.p.x + vx  && (p1y + ry) >= c.p.y+vy && (p2y - ry) <= c.p.y + vy)
        {
			vx *= -1;
			vy *= -1;
		}
		else if(!flag && (p1x + rx) >= c.p.x+vx && (p2x - rx) <= c.p.x + vx  && (p1y + ry) <= c.p.y + vy && (p2y - ry) >= c.p.y + vy)
        {
			vx *= -1;
			vy *= -1;
		}
